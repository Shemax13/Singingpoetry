...
- **Меню починено**: удалён `events.js` (дублировал слушатели `app.js` → меню открывалось и сразу закрывалось)
- **Хамбургер**: стеклянный полупрозрачный дизайн с `backdrop-filter`
- **Меню**: анимация шторки через `clip-path` (curtain slide-down)
- **Экраны**: `aspect-ratio` заменён на `height: 100vh`, три экрана заполняют окно без полей
- **Деплой**: Workers Builds (новый аккаунт) + GitHub Action (старый аккаунт)
